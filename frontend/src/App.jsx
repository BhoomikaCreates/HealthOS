...
function App() {
  ...
  const fetchHealthData = async () => {
    try {
      const response = await fetch(`${BACKEND}/api/health-data`);
      if (!response.ok) throw new Error();
      const data = await response.json();
      if (data.length > 0) {
        const latest = data[0];
        setStats({
          water: `${latest.water} Liters`, 
      ...
      setWeeklyData([
        { name: 'Mon', water: 2.1, sleep: 6 }, 
        { name: 'Tue', water: 2.5, sleep: 7 }, 
        { name: 'Wed', water: 1.8, sleep: 5.5 }, 
        { name: 'Thu', water: 3.0, sleep: 8 }, 
        { name: 'Fri', water: 2.8, sleep: 6.5 }
      ]);
    }
  };
  ...
  const fetchAIInsight = async () => {
    try {
      const response = await fetch(`${BACKEND}/api/ai-insight`);
      const data = await response.json();
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    {[
                      ...
                      { label: "Sleep", val: stats.sleep, icon: Moon, color: "text-purple-400", border: "hover:border-purple-500" }, 
                      { label: "Steps", val: stats.steps, icon: Footprints, color: "text-green-400", border: "hover:border-green-500" }, 
                      { label: "Calories", val: stats.calories, icon: Flame, color: "text-orange-400", border: "hover:border-orange-500" }
                    ].map((item, i) => (
                      <div key={i} className={`bg-slate-900 p-6 rounded-2xl border border-slate-800 transition-all ${item.border}`}>
                        <div className="flex justify-between items-center mb-2">
                          <h3 className="text-gray-400 text-sm font-bold uppercase">{item.label}</h3>
                      ...
                      {weeklyData.length > 0 && (
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={weeklyData}>
                            ...
                            <Area type="monotone" dataKey="steps" stroke="#22c55e" strokeWidth={3} fillOpacity={0} name="Steps" />
                          <Area type="monotone" dataKey="calories" stroke="#f97316" strokeWidth={3} fillOpacity={0} name="Calories (Kcal)" />
                          </AreaChart>
                        </ResponsiveContainer>
                      )}
                    </div>
